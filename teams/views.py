...
class TeamAddMemberView(LoginRequiredMixin, generic.View):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if user_to_add in team.members.all():
                ...
                messages.success(
                    request,
                    f"User @{username} has been successfully added!"
                )
...
class TeamRemoveMemberView(LoginRequiredMixin, generic.View):
    def post(self, request, *args, **kwargs):
        team = get_object_or_404(Team, pk=kwargs["pk"])
        ...
        user_to_kick = get_object_or_404(
            User,
        ...
        if user_to_kick == team.owner:
            ...
            messages.success(
                request,
                f"User @{user_to_kick.username} has been removed from the team."
            )

        return redirect(team.get_absolute_url())


class TeamUpdateDescriptionView(LoginRequiredMixin, generic.View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages.success(
            request,
            "Team description has been successfully updated."
        )
